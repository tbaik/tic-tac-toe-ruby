...
class TTTUI
  ...
  def receive_and_set_language_choice
  ...
  def receive_save_file_name
    receive_input_with_checker(@text[:ask_file_name], :valid_file_name?, @text[:invalid_file_name])
  end
  ...
  def receive_board_size
    receive_input_with_checker(@text[:ask_board_size], :valid_board_size?, @text[:invalid_input])
  end
  ...
  def receive_read_or_new_game
    receive_input_with_checker(@text[:ask_read_or_new_game], :valid_read_or_new_game_input?, @text[:invalid_input])
  end

  def receive_read_file_name
    receive_input_with_checker(@text[:ask_file_name], :existing_file_name?, @text[:invalid_file_name])
  end
  ...
  def receive_input_with_checker(prompt_text, checker_method, invalid_text)
    @io.print_message(prompt_text)
    input = @io.get_input
    if @input_checker.send(checker_method, input)
      ...
      receive_input_with_checker(prompt_text, checker_method, invalid_text)
    end
  end
end
