...
describe HardAI do
  ...
  describe '#choose_move' do
    it 'calls medium_move when there are more than 9 available spaces' do
      new_game = TTTGame.new(GameBoard.new(4), HumanPlayer.new("X"), HardAI.new("O"), ConsoleIO, true) #X is human, O is computer, X goes first
  ...
  describe '#get_score' do
    it 'should get a score of 2 if game is winnable with this move' do
      new_game = TTTGame.new(GameBoard.new(3), HumanPlayer.new("X"), HardAI.new("O"), ConsoleIO, true) #X is human, O is computer, X goes first
      new_game.make_move(2,"X")
      new_game.make_move(5,"O")
      new_game.make_move(8,"X")
