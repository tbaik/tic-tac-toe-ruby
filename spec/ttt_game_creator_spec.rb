...
describe TTTGameCreator do
  ...
  it 'initializes with a new receiver' do
    expect{TTTGameCreator.new(NewTTTReceiver.new(ConsoleIO))}.not_to raise_error
  end

  let(:new_receiver) {NewTTTReceiver.new(ConsoleIO)}
  let(:ttt_creator) {TTTGameCreator.new(new_receiver)}

  describe '#new_game' do
    it 'returns a new game when reading a game' do
      expect(new_receiver).to receive(:receive_read_or_new_game).and_return("1")
      expect(new_receiver).to receive(:receive_piece_and_turn).and_return(["O",true])
      expect(new_receiver).to receive(:receive_difficulty).and_return(EasyAI)
      expect(new_receiver).to receive(:receive_board_size).and_return(3)
      ...
      expect(game.io).to eq(ConsoleIO)
      expect(game.is_player_turn).to eq(true)
    end

    it 'returns a new game when creating a new game from scratch' do
      game = TTTGame.new(GameBoard.new(3), HumanPlayer.new("O"), EasyAI.new("X"), ConsoleIO, true)
      TTTGameWriter.write_game(game, "test_read.txt")
      expect(new_receiver).to receive(:receive_read_file_name).and_return("test_read.txt")
      expect(new_receiver).to receive(:receive_read_or_new_game).and_return("2")
  ...
  describe '#create_new_game' do
    it 'takes a variable hash and creates a new game with gb_size' do
      variables_hash = {:gb_size => 3, :hp_piece => "O", :cp_piece => "X", :cp_class => EasyAI, 
        :io => ConsoleIO, :is_player_turn => true} 
  ...
  describe '#new_game_variables' do
    it 'receives all user input and turns it into correct hash of variables' do
      ...
      expect(variables_hash[:io]).to eq(ConsoleIO)
